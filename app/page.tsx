...
function Report({companyName,readiness,resolved,demoLoaded,setView}:{companyName:string;readiness:number;resolved:boolean;demoLoaded:boolean;setView:(v:View)=>void}) {
  ...
  return <div className="content report-page"><div className="page-intro"><div><h2>小微企业融资准备度测评报告</h2><p>报告编号 RST-2026-0807-001 · 样本模式 · 规则基准日 2026-08-01</p></div><button className="primary" onClick={()=>window.print()}>⇩ 导出完整 PDF</button></div>
    <section className="report-cover"><div><span className="status-pill"><i/> 合规演示样本 · 分析已完成</span><h2>{companyName}</h2><p>融资目标：80 万元 · 设备采购与库存补充 · 期限 24 个月</p><div className="cover-meta"><span>适用制度：小企业会计准则</span><span>分析材料：5 份</span><span>证据链：12 条</span></div></div><div className="report-score"><strong>{readiness}</strong><span>融资准备度</span><small>{resolved?"已完成 1 项解释":"尚有高优先级待确认"}</small></div></section>
    ...
    <section id="finance" className="report-section"><div className="report-heading"><span>05</span><div><small>FINANCING READINESS</small><h2>融资规则匹配</h2></div></div><div className="finance-table"><div className="finance-head"><span>模拟产品</span><span>适配度</span><span>已满足</span><span>主要缺口</span><span>边界</span></div>{productRows.map((p,i)=><div className="finance-row" key={p.name}><strong>{p.name}<small>{p.bank}</small></strong><b>{resolved&&i===2?46:p.fit}%</b><span>{i===0?"经营年限、流水稳定":"主体资料、经营流水"}</span><span>{i===1?"4月纳税记录":i===2?"经营流入口径确认":"主要客户合同"}</span><em>非审批结果</em></div>)}</div></section>
