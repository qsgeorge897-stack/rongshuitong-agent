  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "materials" && <Materials files={files} demoLoaded={demoLoaded} parsedFiles={parsedFiles} parsing={parsing} onDemo={() => {setDemoLoaded(true);setRealAnalysis(null);setParsedFiles([]);setFiles(5)}} onUpload={() => fileRef.current?.click()} onAnalyze={() => setRealAnalysis(summarizeFiles(parsedFiles))} />}
        ...
        {view === "report" && <Report companyName={companyName} readiness={readiness} resolved={resolved} demoLoaded={demoLoaded} realAnalysis={realAnalysis} setView={setView} />}
        {view === "evaluation" && <Evaluation />}
        {view === "standards" && <Standards />}
      </section>

      <input ref={fileRef} className="hidden" type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.webp,.xlsx,.xls,.csv,.txt" onChange={async e => { const selected=Array.from(e.target.files||[]); if(!selected.length)return; setParsing(true);setDemoLoaded(false);setRealAnalysis(null);setView("materials");setFiles(v=>v+selected.length);const initial=selected.map(f=>({id:`${Date.now()}-${f.name}`,name:f.name,kind:"其他" as const,status:"解析中" as const,rows:0,text:"",fields:{},evidence:[]}));setParsedFiles(v=>[...v,...initial]);const results=[];for(const file of selected)results.push(await parseRealFile(file));setParsedFiles(v=>[...v.filter(old=>!initial.some(i=>i.name===old.name)),...results]);setParsing(false);e.target.value=""; }} />
    </main>
  );
}
...
function Materials({ files, demoLoaded, parsedFiles, parsing, onDemo, onUpload, onAnalyze }: { files: number; demoLoaded:boolean;parsedFiles:ParsedFile[];parsing:boolean;onDemo:()=>void; onUpload: () => void;onAnalyze:()=>void }) {
  return <div className="content"><div className="page-intro"><div><h2>企业材料中心</h2><p>上传材料后，系统会自动分类、抽取关键字段并等待你的确认。</p></div><button className="primary" onClick={onUpload}>＋ 上传新材料</button></div>
    ...
    <section className="panel table-panel"><div className="panel-title"><div><span className="section-kicker">已提交 {files} 份</span><h3>{demoLoaded?"演示样本识别结果":"真实文件本地解析"}</h3></div><span className="success-text">● 文件仅在当前浏览器处理 · 不上传服务器</span></div>{files===0?<div className="empty-state"><strong>尚未提交材料</strong><p>请先上传至少一份企业材料，系统不会默认代入任何公司或经营数据。</p></div>:demoLoaded?<div className="doc-table"><div className="table-head"><span>材料名称</span><span>类别</span><span>识别摘要</span><span>核验状态</span></div>{documents.map((d,i)=><div className="table-row" key={d.name}><span><i className="file-icon">{i===1?"X":"P"}</i><b>{d.name}</b></span><span>{d.type}</span><span>{d.meta}</span><span><em className={d.state.includes("已核验")?"ok":d.state.includes("缺失")?"bad":"pending"}>{d.state}</em></span></div>)}</div>:<RealFileTable files={parsedFiles} parsing={parsing}/>}</section>
    {!demoLoaded&&parsedFiles.length>0&&<div className="analysis-cta"><div><strong>{parsing?"正在解析真实文件…":"解析完成，等待用户确认并生成报告"}</strong><small>Excel/CSV读取表头和金额列，文本PDF提取文字，图片使用中英文OCR。结果不会自动认定融资资格。</small></div><button className="primary" disabled={parsing||parsedFiles.every(f=>f.status==="失败")} onClick={onAnalyze}>生成真实数据报告 →</button></div>}
  </div>;
}

function RealFileTable({files,parsing}:{files:ParsedFile[];parsing:boolean}){return <div className="real-files">{files.map(f=><article className="real-file" key={f.id}><div className="file-main"><i className="file-icon">{f.name.split(".").pop()?.slice(0,1).toUpperCase()}</i><div><strong>{f.name}</strong><small>{f.kind} · {f.rows?`${f.rows} 页/行`:"等待读取"}</small></div><em className={f.status==="已解析"?"ok":f.status==="失败"?"bad":"pending"}>{f.status}</em></div>{f.status==="已解析"||f.status==="需确认"?<div className="field-preview">{Object.entries(f.fields).slice(0,6).map(([k,v])=><span key={k}><b>{k}</b>{typeof v==="number"&&/流入|流出|合计|税额/.test(k)?`¥${v.toLocaleString()}`:String(v)}</span>)}</div>:null}{f.error&&<p className="parse-error">{f.error}</p>}</article>)}{parsing&&<div className="parsing-note"><i/>OCR或文档解析可能需要几十秒，请勿关闭页面。</div>}</div>}
...
function Report({companyName,readiness,resolved,demoLoaded,realAnalysis,setView}:{companyName:string;readiness:number;resolved:boolean;demoLoaded:boolean;realAnalysis:RealAnalysis|null;setView:(v:View)=>void}) {
  if(realAnalysis)return <RealDataReport companyName={companyName} data={realAnalysis}/>;
  if(!demoLoaded)return <div className="content"><div className="report-empty"><span>▥</span><h2>尚未生成测评报告</h2><p>系统不会在没有完成材料解析和规则核验时生成预设结论。请上传真实文件并点击“生成真实数据报告”，或载入明确标记的合规演示样本。</p><button className="primary" onClick={()=>setView("materials")}>前往材料中心 →</button></div></div>;
...
function RealDataReport({companyName,data}:{companyName:string;data:RealAnalysis}){const has=(k:string)=>data.kinds.includes(k);const parsed=data.files.filter(f=>f.status==="已解析").length;const completeness=Math.round((["营业执照","银行流水","发票明细","财务报表","纳税资料","合同"].filter(has).length/6)*100);const net=data.income-data.expense;return <div className="content report-page"><div className="page-intro"><div><h2>真实材料融资准备预核验报告</h2><p>本地解析 · {data.parsedAt} · {data.files.length}份文件</p></div><button className="primary" onClick={()=>window.print()}>⇩ 导出当前报告</button></div><section className="report-cover real-cover"><div><span className="status-pill"><i/> 真实文件模式</span><h2>{companyName}</h2><p>本报告中的金额仅来自本次上传文件可识别字段，未识别内容不会被推测填充。</p><div className="eligibility-banner"><b>预核验结论：{completeness>=80?"材料基本齐备，仍需银行外部核验":"材料不足，暂无法判断是否具备融资申请条件"}</b><span>当前覆盖 {data.kinds.length} 类材料；完整度 {completeness}%。信用、行政处罚、许可状态及材料真实性不在本地文件解析能力范围内。</span></div></div><div className="report-score"><strong>{completeness}</strong><span>材料覆盖度</span><small>不等于授信通过率</small></div></section><section className="report-section"><div className="report-heading"><span>01</span><div><small>ACTUAL FILE EXTRACTION</small><h2>真实文件解析结果</h2></div></div><div className="real-summary"><div><span>成功解析</span><strong>{parsed} / {data.files.length}</strong></div><div><span>材料类别</span><strong>{data.kinds.length}</strong></div><div><span>识别经营流入</span><strong>{data.income?`¥${data.income.toLocaleString()}`:"未识别"}</strong></div><div><span>识别经营流出</span><strong>{data.expense?`¥${data.expense.toLocaleString()}`:"未识别"}</strong></div><div><span>识别发票合计</span><strong>{data.invoiceTotal?`¥${data.invoiceTotal.toLocaleString()}`:"未识别"}</strong></div><div><span>识别税额合计</span><strong>{data.taxTotal?`¥${data.taxTotal.toLocaleString()}`:"未识别"}</strong></div></div>{data.income>0&&data.expense>0&&<div className="formula-panel"><div><span>可复算指标</span><strong>经营性净流入 = 已识别经营流入 − 已识别经营流出</strong></div><code>{data.income.toLocaleString()} − {data.expense.toLocaleString()} = {net.toLocaleString()} 元</code><p>仅当上传表格包含明确的收入/支出或贷方/借方字段时计算。</p></div>}<div className="actual-file-list">{data.files.map(f=><div key={f.id}><strong>{f.name}<small>{f.kind} · {f.status}</small></strong><span>{Object.entries(f.fields).slice(0,5).map(([k,v])=>`${k}: ${v}`).join(" ｜ ")||"没有可确认字段"}</span><em>{f.evidence[0]||"无文本证据"}</em></div>)}</div></section><section className="report-section"><div className="report-heading"><span>02</span><div><small>REGULATORY PRE-CHECK</small><h2>融资申请条件逐项预核验</h2></div></div><div className="authority-note"><strong>判断基准</strong><p>《流动资金贷款管理办法》第十四条。这里只判断上传材料是否提供了支持性证据，不替代银行的真实性核验、征信查询、尽职调查和授信审批。</p><a href="https://www.nfra.gov.cn/cn/view/pages/ItemDetail.html?docId=1151066&generaltype=0&itemId=925" target="_blank" rel="noreferrer">监管原文 ↗</a></div><div className="qualification-list"><Qualification status={has("营业执照")?"pass":"review"} no="01" title="依法登记" basis="借款人依法经市场监督管理部门或主管部门核准登记。" evidence={has("营业执照")?"已识别营业执照类材料。":"未识别营业执照类材料。"} conclusion={has("营业执照")?"材料提供初步支持":"无法判断"} caveat="仍需查询企业存续状态、经营异常名录及登记信息一致性。"/><Qualification status="review" no="02" title="借款用途明确、合法" basis="用途应明确、合法，且不得将流动资金贷款用于固定资产或股权投资等限制用途。" evidence="当前上传文件解析未获得经用户确认的融资金额、用途及支付对象。" conclusion="无法判断" caveat="需填写融资用途并关联合同、订单或采购计划；涉及设备购置时应确认是否属于固定资产融资。"/><Qualification status="review" no="03" title="经营合法、合规" basis="借款人经营应合法、合规。" evidence={has("营业执照")?"营业执照仅能支持经营范围核对，不能证明不存在行政处罚。":"缺少主体及外部合规信息。"} conclusion="待外部核验" caveat="需核验许可、行政处罚和行业监管信息。"/><Qualification status={data.income>0&&data.expense>0?"pass":"review"} no="04" title="持续经营能力与合法还款来源" basis="借款人应具有持续经营能力和合法还款来源。" evidence={data.income>0&&data.expense>0?`文件可复算经营流入${data.income.toLocaleString()}元、流出${data.expense.toLocaleString()}元、净流入${net.toLocaleString()}元。`:"当前文件未提供可计算的经营流入和流出字段。"} conclusion={data.income>0&&data.expense>0&&net>0?"样本期现金净流入提供初步支持":"证据不足"} caveat="仍需确认统计期间、交易分类、存量负债和未来现金流；正净流入不等于具备偿债能力。"/><Qualification status="review" no="05" title="信用状况良好" basis="借款人信用状况应良好。" evidence="本系统不读取或推断企业及个人征信。" conclusion="无法判断" caveat="须由持牌银行取得授权后依法查询。"/><Qualification status={completeness>=80?"pass":"review"} no="06" title="材料真实、完整、有效" basis="借款人应承诺所提供材料真实、完整、有效。" evidence={`已上传${data.files.length}份、覆盖${data.kinds.length}类材料；当前材料覆盖度${completeness}%。`} conclusion={completeness>=80?"具备提交前复核基础":"材料不完整"} caveat="机器识别不验证文件真实性；所有抽取字段必须由用户确认。"/></div></section><section className="report-section"><div className="report-heading"><span>03</span><div><small>GAPS & NEXT ACTIONS</small><h2>缺口与下一步</h2></div></div><div className="action-timeline">{[[!has("营业执照"),"补充营业执照","用于主体登记信息核对"],[!has("银行流水"),"补充银行流水","用于现金流和还款来源分析"],[!has("发票明细"),"补充发票明细","用于收入与交易证据勾稽"],[!has("纳税资料"),"补充纳税资料","用于税务类产品材料准备"]].map((x,i)=><div key={String(x[1])}><span>{i+1<3?"高优先级":"申请前"}</span><strong>{x[0]?x[1]:`${String(x[1]).replace("补充","")}已提供`}</strong><p>{x[2]}</p><em className={x[0]?"pending":"ok"}>{x[0]?"待补充":"已覆盖"}</em></div>)}</div></section><section className="disclaimer"><strong>真实文件模式说明</strong><p>文件在当前浏览器中解析，本版本不会把原始文件上传服务器。Excel/CSV依据字段名汇总，文本PDF提取文字，图片使用OCR。OCR和自动分类可能出错；报告不验证材料真伪，不查询征信，不构成贷款审批、会计、税务或法律意见。</p></section></div>}
